 ...
 * {@code userPrincipalName}</a> or a custom
 * {@link #setSearchFilter(String) searchFilter} in the form
 * {@code username@domain}. If the username does not already end with the domain
 * name, the {@code userPrincipalName} will be built by appending the configured
 * domain name to the username supplied in the authentication request. If no
 * domain name is configured, it is assumed that the username will always
 ...
 * @since 3.1
 */
public final class ADAuthenticationProvider extends
     ...
     */
    public ADAuthenticationProvider(String domain, String url,
            String rootDn) {
    ...
    private void raiseExceptionForErrorCode(int code, NamingException exception) {
        ...
        Throwable cause = new ADAuthenticationException(hexString,
    ...
    private DirContextOperations searchForUser(DirContext context, String username)
        ...
        try {
            this.logger.info((Object)("The username is Axis XXXXX: " + username));
            return SpringSecurityLdapTemplate.searchForSingleEntryInternal(context,
                    searchControls, searchRoot, this.searchFilter,
                    new Object[]{username});
     ...
     * failed bind attempt will be parsed for the AD-specific error code and a null     {@link CredentialsExpiredException}, {@link DisabledException},
     ...
     * Occurrences of {1} are replaced with the {@code username} only.
     * <p>
