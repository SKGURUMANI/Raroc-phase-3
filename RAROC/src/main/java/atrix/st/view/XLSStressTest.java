...
import atrix.common.view.admin.*;
 ...
 * @author vaio
 */
public class XLSStressTest extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document,
        ...
        response.setHeader("Content-Disposition", "attachment; filename=\"SysLog.pdf\"");
        float[] colsWidth = {1.667f, 1.667f, 1.667f, 1.667f, 1.667f, 1.667f};

        HeaderFooter heading = new HeaderFooter(new Phrase(getMessageSourceAccessor().getMessage("caption.admin.sysLog")), false);
        ...
        header = new PdfPCell(new Phrase(getMessageSourceAccessor().getMessage("label.result"), smallBold));
        header.setHorizontalAlignment(Element.ALIGN_LEFT);
        header.setVerticalAlignment(Element.ALIGN_MIDDLE);
        header.setBackgroundColor(Color.lightGray);
        table.addCell(header);

        header = new PdfPCell(new Phrase(getMessageSourceAccessor().getMessage("label.ipAddress"), smallBold));
        header.setHorizontalAlignment(Element.ALIGN_LEFT);
        header.setVerticalAlignment(Element.ALIGN_MIDDLE);
        header.setBackgroundColor(Color.lightGray);
        table.addCell(header);

        header = new PdfPCell(new Phrase(getMessageSourceAccessor().getMessage("label.time"), smallBold));
        header.setHorizontalAlignment(Element.ALIGN_LEFT);
        header.setVerticalAlignment(Element.ALIGN_MIDDLE);
        header.setBackgroundColor(Color.lightGray);
        table.addCell(header);
        ...
        while (itr1.hasNext()) {
            ...
            tdrow = new PdfPCell(new Phrase(object.getUserId(), small));
            ...
            tdrow = new PdfPCell(new Phrase(fmt.ToString(object.getRoles()), small));
            tdrow.setHorizontalAlignment(Element.ALIGN_LEFT);
            tdrow.setVerticalAlignment(Element.ALIGN_MIDDLE);
            table.addCell(tdrow);

            tdrow = new PdfPCell(new Phrase(fmt.ToString(object.getAddress()), small));
            tdrow.setHorizontalAlignment(Element.ALIGN_LEFT);
            tdrow.setVerticalAlignment(Element.ALIGN_MIDDLE);
            table.addCell(tdrow);

            tdrow = new PdfPCell(new Phrase(fmt.ToString(object.getActiveStr()), small));
            tdrow.setHorizontalAlignment(Element.ALIGN_LEFT);
            tdrow.setVerticalAlignment(Element.ALIGN_MIDDLE);
            table.addCell(tdrow);

            tdrow = new PdfPCell(new Phrase(fmt.ToString(object.getSessionTime()), small));
            tdrow.setHorizontalAlignment(Element.ALIGN_LEFT);
            tdrow.setVerticalAlignment(Element.ALIGN_MIDDLE);
            table.addCell(tdrow);
